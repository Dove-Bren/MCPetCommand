...
public class PetInfo {
	...
	protected void set(EPetAction action, double hp, double maxHp, PetValue ... values) {
		this.currentHp = hp;
		this.maxHp = maxHp;
		...
		this.action = action == null ? EPetAction.WAIT : action;
	}
	
	protected void set(EPetAction action, double hp, double maxHp, double secondary, double maxSecondary, ValueFlavor flavor, Component secondaryLabel) {
	...
	protected void set(double hp, double maxHp, EPetAction action) {
	...
	public static final PetInfo claim(EPetAction action, double hp, double maxHp, PetValue ...values) {
		PetInfo info = claim();
		info.set(action, hp, maxHp, values);
		return info;
	}
	
	public static final PetInfo claim(EPetAction action, double hp, double maxHp, double secondary, double maxSecondary, ValueFlavor flavor, Component secondaryLabel) {
		PetInfo info = claim();
	...
	public static final PetInfo claim(double hp, double maxHp, EPetAction action) {
		PetInfo info = claim();
	...
	public static PetInfo WrapVanilla(LivingEntity entity) {
		if (entity instanceof TamableAnimal tame) {
			final @Nullable LivingEntity target = PetCommandAPI.GetTargetManager(entity).getTarget((TamableAnimal) entity);
			final @Nullable EPetAction order = PetInfo.GetOrderAction(entity);
			final EPetAction action = 
					  order != null ? order
					: tame.isInSittingPose() ? EPetAction.STAY
					: (target != null && target.isAlive()) ? EPetAction.ATTACK		
					: EPetAction.IDLE;
	...
	/**
	 * Fetch the PetInfo for the given entity. Works with IEntityPet or other mobs that don't implement the interface,
	 * with an autogenerated summary made for them.
	 * @param entity
	 * @return
	 */
	public static PetInfo Wrap(LivingEntity entity) {
		if (entity instanceof IEntityPet p) {
			return p.getPetSummary();
		} else {
			return PetInfo.WrapVanilla(entity);
		}
	}
	
	/**
	 * Check and return an action describing the current pet order, if there is one.
	 * Returns null if no order is active for this pet, indicating the pet should
	 * formulate it's own summary of its current action.
	 * 
	 * It is HIGHLY recommended you use this when creating a PetInfo for a consistent pet experience.
	 * @param entity
	 * @return
	 */
	public static @Nullable EPetAction GetOrderAction(LivingEntity entity) {
		LivingEntity owner = PetFuncs.GetOwner(entity);
		if (owner == null) {
			return null;
		}
		@Nullable EPetOrderType order = PetCommandAPI.GetPetOrderManager().getCurrentOrder(owner, entity);
		if (order != null) {
			switch (order) {
			case GUARD_POS:
				return EPetAction.GUARD;
			case MOVE_TO_ME:
			case MOVE_TO_POS:
			default:
				return EPetAction.MOVE;
			case STAY:
				return EPetAction.STAY;
			}
		}
	...
	public static class ManagedPetInfo extends PetInfo {
		...
		@Override
		public void set(EPetAction action, double hp, double maxHp, PetValue ...values) {
			super.set(action, hp, maxHp, values);
		}
		
		@Override
		public void set(EPetAction action, double hp, double maxHp, double secondary, double maxSecondary, ValueFlavor flavor, Component secondaryLabel) {
		...
		@Override
		public void set(double hp, double maxHp, EPetAction action) {
			super.set(hp, maxHp, action);
		}
		
		@Override
		public void set(double hp, double maxHp) {
			super.set(hp, maxHp, (EPetAction)null);
		}
